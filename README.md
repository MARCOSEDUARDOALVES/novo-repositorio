...
O projeto agora utiliza o fluxo padrão do React (Create React App) para o
front-end e mantém um servidor Express simples para distribuir a versão
compilada. O passo a passo abaixo funciona no Windows (Git Bash), macOS ou
Linux:
...
3. **Desenvolvimento com recarregamento automático (React)**
   ```bash
   npm start
   ```
   - O comando executa o `react-scripts start`, abrindo `http://localhost:3000`
     (ou a porta seguinte disponível) no navegador.
   - Qualquer alteração em arquivos dentro de `src/` é recarregada
     automaticamente.

4. **Gerar a versão otimizada e servir pelo backend Express**
   ```bash
   npm run build
   npm run server
   ```
   - `npm run build` cria a pasta `build/` com os arquivos estáticos prontos.
   - `npm run server` executa `server.js`, que serve o conteúdo compilado em
     `http://localhost:4000`.
   - O terminal exibirá também os endereços IPv4 disponíveis na mesma rede
     (ex.: `http://192.168.0.10:4000`), facilitando o acesso por outros
...
> comando `npm start` (ou outro script) é executado. Antes de rodar qualquer
> script do `npm`, confirme que o caminho atual contém `package.json`:
>
> ```bash
...
> arquivos como `package.json` e `src/App.jsx` estão visíveis; só então execute os
> comandos do React.

5. **Encerrar o servidor ou o ambiente de desenvolvimento**
   - Pressione `Ctrl + C` no terminal para finalizar o processo em execução.
...
> sua preferência) apontando para a porta exibida pelo servidor (`4000` por
> padrão).
