...
OUTPUT_SPEC = [
    ...
    },
    {
        ...
        "description": "Características astrológicas derivadas (graus, dispositores e perfis de temperamento tradicionais/modernos).",
    },
    {
...
def export_offline_bundle() -> list[dict[str, str]]:
...
def main() -> None:
    ...
    parser.add_argument(
        "--no-zip",
        action="store_true",
        help="Não gera o arquivo offline_bundle.zip (por padrão ele é criado).",
    )
    args = parser.parse_args()

    exported_files = export_offline_bundle()
    if not args.no_zip and exported_files:
        build_zip_archive()
