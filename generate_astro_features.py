...
def _load_reduced_dataframe() -> "DataFrame":
    if pd is None:
        raise RuntimeError("Pandas é obrigatório para carregar a base reduzida.")

    if INPUT_FILE.exists():
        logging.info("Carregando dados reduzidos de %s", INPUT_FILE)
        return pd.read_csv(INPUT_FILE)

    try:
        client = AstroDatabaseClient()
        records = client.fetch_reduced_people()
        if not records:
            raise AstroDatabaseError("Nenhum registro reduzido retornado pelo banco astrológico.")
        df_remote = pd.DataFrame(records)
        logging.info(
            "Dados reduzidos carregados diretamente do banco astrológico (%s registros).",
            len(df_remote),
        )
        return df_remote
    except AstroDatabaseError as exc:
        logging.warning("Falha ao obter dados reduzidos do banco astrológico: %s", exc)

    logging.warning(
        "Arquivo %s não encontrado. Utilizando entrada de amostra %s.",
        INPUT_FILE.name,
        SAMPLE_INPUT_FILE,
    )
...
def main() -> None:
    ...
    df_reduced = _load_reduced_dataframe()
