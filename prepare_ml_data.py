...
def main() -> None:
    ...
    try:
        ...
        if source_file == SAMPLE_INPUT_FILE:
            logging.warning(
                "Arquivo %s não encontrado. Utilizando dados de amostra %s.", INPUT_FILE.name, source_file
            )
        else:
            logging.info("Dados carregados do arquivo: %s", source_file)

        df_astro = pd.read_csv(source_file)
